...
def project_IP_form_cash0_beta1(
    ...
    for t, w in W.iterrows():
        ...
        UtU = np.array([[k, bv.sum()], [bv.sum(), np.dot(bv, bv)]], dtype=float)
        ...
        invUtU = (1.0 / det) * np.array(
            [[UtU[1, 1], -UtU[0, 1]], [-UtU[1, 0], UtU[0, 0]]], dtype=float
        )
...
def backtest(
    ...
    if vol_scale:
        ...
        cumulative_resid = (1 + scaled_sector_resid).rolling(
            window=signal_window
        ).apply(np.prod, raw=True) - 1
        daily_reverse_signal = -cumulative_resid
    else:
        cumulative_resid = (1 + sector_resid).rolling(window=signal_window).apply(
            np.prod, raw=True
        ) - 1
        daily_reverse_signal = -cumulative_resid

    z_scores_group = daily_reverse_signal.sub(
        cumulative_resid.mean(axis=1), axis=0
    ).div(cumulative_resid.std(axis=1), axis=0)
    if sector_neutral:
    ...
    short_weights = (
        short_signals.div(short_signals.abs().sum(axis=1), axis=0).fillna(0) * 1 / 2
    )
    if not beta_0:
        long_weights = (
            long_signals.div(long_signals.sum(axis=1), axis=0).fillna(0) * 1.5
        )
        short_weights = (
            short_signals.div(short_signals.abs().sum(axis=1), axis=0).fillna(0) * 1 / 2
        )
    ...
    if exponential_weights:
        ...
        if beta_0:
            long_weights = (
                long_weights.div(long_weights.sum(axis=1), axis=0).fillna(0) * 1 / 2
            )
            short_weights = (
                short_weights.div(short_weights.abs().sum(axis=1), axis=0).fillna(0)
                * 1
                / 2
            )
        elif not beta_0:
            long_weights = (
                long_weights.div(long_weights.sum(axis=1), axis=0).fillna(0) * 1.5
            )
            short_weights = (
                short_weights.div(short_weights.abs().sum(axis=1), axis=0).fillna(0)
                * 1
    ...
    if beta_0:
        ...
        short_weights = (
            short_weights.div(short_weights.abs().sum(axis=1), axis=0).fillna(0) * 1 / 2
        )
        ...
        long_weights = (
            long_weights.div(long_weights.sum(axis=1), axis=0).fillna(0) * 1.1
        )
        short_weights = (
            short_weights.div(short_weights.abs().sum(axis=1), axis=0).fillna(0)
            * 1
